...
/*   Updated: 2019/04/03 13:38:39 by fcordon     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
#define ERR1	"indirect value can't be a register"
#define ERR2	"undefined label"
#define ERR3	"unexpected token"
...
static char		*indirect_value(char *s, t_data *d, t_child *c, int i)
{
	...
	if (!(j = is_label(++s, d->label, d)))
	{
		...
		{
			p.col += 1;
			return (perror_and_set_fmt(ERR1, &p, d, d->current->fmt + i));
		}
		if (ft_isalpha(*s))
		{
			p.col += s - c->s;
			return (perror_and_set_fmt(ERR2, &p, d, d->current->fmt + i));
		}
		if (!ft_isdigit(*s))
			return (IND_ERROR);
		while (ft_isdigit(*(++s)))
...
static char		*direct_value(char *s, t_data *d, t_child *c, int i)
{
	...
	if (!(j = is_label(s, d->label, d)))
	{
		...
		{
			p.col += s - c->s;
			perror_and_set_fmt(ERR3, &p, d, d->current->fmt + i);
			return (DIR_ERROR);
		}
		while (ft_isdigit(*(++s)))
			;
